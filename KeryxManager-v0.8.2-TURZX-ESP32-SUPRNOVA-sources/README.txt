...
- Optional keryx-miner-supr Windows NVIDIA package for RTX 40/50 GPUs.
...
- Optional Wi-Fi dashboard on the Waveshare ESP32-S3-LCD-4.3 non-touch display.
...
3. Optional Suprnova: keep the complete package together in "miner-suprnova",
   or choose keryx-miner-supr.exe with the Browse button.
4. For solo mining, put keryxd.exe in "keryxd".
5. Start KeryxControl.exe.
...
- Local display-only ESP32 telemetry every two seconds; no remote mining controls.
- Solo mining through a managed local keryxd node.
- Keryx Stratum v3 pool mode.
- Optional Suprnova profile for NVIDIA RTX 40/50 GPUs, with wallet.worker,
  endpoint, local statistics API and executable selection.
...
SUPRNOVA MODE
-------------
Select "Suprnova - NVIDIA RTX 40/50 (pool)" under Mining software. The default
endpoint is stratum+tcp://krx.suprnova.cc:4404. TCP and SSL Stratum URLs with an
explicit port are accepted. Enter a worker name containing 1 to 32 letters,
digits, hyphens or underscores.

The complete keryx-miner-supr-windows-nvidia-pom folder must remain together.
Keryx Control detects it in "miner-suprnova" or in the standard Downloads
folder; Browse can select it elsewhere. The validated package reports version
0.13.3. Keryx Control does not redistribute these miner/CUDA files.

The first model download and preparation can remain at 0 H/s for several
minutes. The v0.8.2 monitor grants a 30-minute preparation window before normal
zero-hashrate alerting resumes. No automatic restart is performed.
...
- The application contains no remote-control web server and sends no telemetry
  to the project author or an Internet service. The optional ESP32 companion
  receives only display statistics broadcast locally over UDP port 42100; no
  wallet, Wi-Fi credential, authorization data, peer or log line is included.
  Settings stay under %LOCALAPPDATA%\KeryxControl and node data stays in KeryxData.
