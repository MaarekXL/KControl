...
public sealed class MainViewModel : ObservableObject, IAsyncDisposable
{
    ...
    private readonly Esp32TelemetryService _esp32Telemetry = new();
    ...
    private MinerConfig _activeMinerConfig = new();
    private MinerBackend _activeMinerBackend = MinerBackend.Reference;
    ...
    private string _lastNodeLine = "", _lastMinerLine = "", _language = "fr", _wallet = "", _nodeAddress = "127.0.0.1", _poolAddress = "", _suprnovaMinerPath = "", _suprnovaWorker = "rig01", _suprnovaPoolAddress = "stratum+tcp://krx.suprnova.cc:4404", _status = "", _statusKey = "StatusInitializing", _statusForeground = "#8BFFAE", _statusDot = "#22E66D";
    ...
    private MinerOptionViewModel? _selectedMiner;
    ...
    private DateTime _minerStartedAtUtc = DateTime.MinValue;
    ...
    public ObservableCollection<MinerOptionViewModel> MinerOptions { get; } = [];
    ...
    public string SuprnovaMinerPath
    {
        get => _suprnovaMinerPath;
        set
        {
            if (!Set(ref _suprnovaMinerPath, value?.Trim() ?? "")) return;
            Raise(nameof(SuprnovaMinerStatusText));
            Raise(nameof(SuprnovaMinerStatusForeground));
            RefreshCommands();
        }
    }
    public string SuprnovaWorker { get => _suprnovaWorker; set { Set(ref _suprnovaWorker, value); RefreshCommands(); } }
    public string SuprnovaPoolAddress { get => _suprnovaPoolAddress; set { Set(ref _suprnovaPoolAddress, value); RefreshCommands(); } }
    public string SuprnovaMinerStatusText => SuprnovaExecutableExists()
        ? T("SuprnovaMinerReady")
        : T("SuprnovaMinerMissing");
    public string SuprnovaMinerStatusForeground => SuprnovaExecutableExists() ? "#79D895" : "#F2C66D";
    ...
    public bool MiningModeConfigurationEnabled => ConnectionConfigurationEnabled && !IsSuprnovaMiner;
    public MinerOptionViewModel? SelectedMiner
    {
        get => _selectedMiner;
        set
        {
            if (!Set(ref _selectedMiner, value)) return;
            if (IsSuprnovaMiner)
                SelectedMiningMode = MiningModeOptions.FirstOrDefault(x => x.Id.Equals("pool", StringComparison.OrdinalIgnoreCase));
            Raise(nameof(IsSuprnovaMiner)); Raise(nameof(IsStandardMiner)); Raise(nameof(IsStandardPoolMode));
            Raise(nameof(MiningModeConfigurationEnabled)); Raise(nameof(SuprnovaMinerStatusText)); Raise(nameof(SuprnovaMinerStatusForeground));
            RefreshNodeSyncText(); RefreshCommands();
        }
    }
    public bool IsSuprnovaMiner => SelectedMiner?.Id.Equals("suprnova", StringComparison.OrdinalIgnoreCase) == true;
    public bool IsStandardMiner => !IsSuprnovaMiner;
    public MiningModeOptionViewModel? SelectedMiningMode
    {
        ...
        set
        {
            ...
            Raise(nameof(IsSoloMode)); Raise(nameof(IsPoolMode)); Raise(nameof(IsStandardPoolMode)); Raise(nameof(NodeControlsEnabled)); Raise(nameof(StartButtonText));
            RefreshNodeSyncText(); RefreshCommands();
            if (!_miner.IsRunning) SetStatus("StatusReady");
        }
    }
    public bool IsPoolMode => IsSuprnovaMiner || SelectedMiningMode?.Id.Equals("pool", StringComparison.OrdinalIgnoreCase) == true;
    public bool IsSoloMode => !IsPoolMode;
    public bool IsStandardPoolMode => IsPoolMode && !IsSuprnovaMiner;
    ...
    {
        MinerOptions.Add(new("reference", "Keryx Miner 0.5.4-PoM — solo / pool", "Keryx Miner 0.5.4-PoM — solo / pool", _language));
        MinerOptions.Add(new("suprnova", "Suprnova — NVIDIA RTX 40/50 (pool)", "Suprnova — NVIDIA RTX 40/50 (pool)", _language));
        _selectedMiner = MinerOptions[0];
    ...
    public async Task InitializeAsync()
    {
        ...
        SuprnovaWorker = IsValidWorker(_settings.SuprnovaWorker) ? _settings.SuprnovaWorker : DefaultWorkerName();
        SuprnovaPoolAddress = string.IsNullOrWhiteSpace(_settings.SuprnovaPoolAddress)
            ? "stratum+tcp://krx.suprnova.cc:4404"
            : _settings.SuprnovaPoolAddress;
        ...
        SuprnovaMinerPath = string.IsNullOrWhiteSpace(_settings.SuprnovaMinerPath)
            ? FindSuprnovaMinerPath()
            : _settings.SuprnovaMinerPath;
        ...
        SelectedMiner = MinerOptions.FirstOrDefault(x => x.Id.Equals(_settings.MinerBackend, StringComparison.OrdinalIgnoreCase)) ?? MinerOptions[0];
        await DetectAsync();
        QueueTurzxFrame();
    ...
    private async Task LoadConfigAsync()
    {
        ...
        NormalizeTiers();
        _activeMinerConfig = _config.Miner;
    }

    private void NormalizeTiers()
    {
        _config.Miner ??= new MinerConfig();
        _config.SuprnovaMiner ??= new MinerConfig { Executable = "miner-suprnova/keryx-miner-supr.exe", StatsPort = 4067 };
    ...
    {
        if (_miner.IsRunning || !Gpus.Any(x => x.IsSelected) || !IsValidWallet(Wallet) || !ActiveMinerExecutableExists())
            return false;
        if (IsSuprnovaMiner && !IsValidWorker(SuprnovaWorker)) return false;
        if (IsPoolMode)
            return TryParsePoolEndpoint(ActivePoolAddress, out _, out _);
    ...
    internal static bool IsValidWorker(string? value) => MinerService.IsValidSuprnovaWorker(value);

    private string ActivePoolAddress => IsSuprnovaMiner ? SuprnovaPoolAddress : PoolAddress;

    private MinerConfig RequestedMinerConfig => IsSuprnovaMiner ? _config.SuprnovaMiner : _config.Miner;

    private bool ActiveMinerExecutableExists()
    {
        try
        {
            var executable = IsSuprnovaMiner
                ? MinerService.ResolveExecutablePath(SuprnovaMinerPath)
                : MinerService.ResolveExecutablePath(_config.Miner.Executable, "keryx-miner.exe");
            return File.Exists(executable);
        }
        catch { return false; }
    }

    private bool SuprnovaExecutableExists()
    {
        try { return File.Exists(MinerService.ResolveExecutablePath(SuprnovaMinerPath)); }
        catch { return false; }
    }

    private string FindSuprnovaMinerPath()
    {
        var candidates = new[]
        {
            _config.SuprnovaMiner.Executable,
            Path.Combine(Environment.GetFolderPath(Environment.SpecialFolder.UserProfile), "Downloads",
                "keryx-miner-supr-windows-nvidia-pom", "keryx-miner-supr.exe")
        };
        foreach (var candidate in candidates)
        {
            try
            {
                var resolved = MinerService.ResolveExecutablePath(candidate);
                if (File.Exists(resolved)) return resolved;
            }
            catch { }
        }
        return _config.SuprnovaMiner.Executable;
    }

    private static string DefaultWorkerName()
    {
        var normalized = Regex.Replace(Environment.MachineName, @"[^a-zA-Z0-9_-]", "-").Trim('-', '_');
        if (normalized.Length > 32) normalized = normalized[..32];
        return IsValidWorker(normalized) ? normalized : "rig01";
    }
    ...
    private async Task StartAsync()
    {
        try
        {
            ...
            if (IsSuprnovaMiner && !IsValidWorker(SuprnovaWorker))
            {
                SetStatus("StatusStartFailed");
                AddApplicationLog(T("LogInvalidSuprnovaWorker"), LogSeverity.Warning);
                return;
            }
            MinerConnection connection;
            if (IsPoolMode)
            {
                if (!TryParsePoolEndpoint(ActivePoolAddress, out var poolHost, out var poolPort))
                {
                    SetStatus("StatusStartFailed");
                    AddApplicationLog(T("LogInvalidPool"), LogSeverity.Warning);
                    return;
                }
                connection = MinerConnection.Pool(ActivePoolAddress.Trim());
            ...
            var requestedConfig = RequestedMinerConfig;
            var requestedBackend = IsSuprnovaMiner ? MinerBackend.Suprnova : MinerBackend.Reference;
            var originalStatsPort = requestedConfig.StatsPort;
            requestedConfig.StatsPort = FindAvailableStatsPort(originalStatsPort);
            if (requestedConfig.StatsPort != originalStatsPort)
                AddApplicationLog(string.Format(T("LogStatsPortChanged"), originalStatsPort, requestedConfig.StatsPort), LogSeverity.Warning);
            ...
            _activeMinerConfig = requestedConfig;
            _activeMinerBackend = requestedBackend;
            _minerStartedAtUtc = DateTime.UtcNow;
            await _miner.StartAsync(requestedConfig, requestedBackend, Wallet.Trim(), connection, launch,
                IsSuprnovaMiner ? SuprnovaMinerPath : null,
                IsSuprnovaMiner ? SuprnovaWorker.Trim() : null,
                _lifetime.Token);
            SetStatus("StatusMiningStarting");
            AddApplicationLog(T(IsSuprnovaMiner ? "LogSuprnovaMinerStarted" : IsPoolMode ? "LogPoolMinerStarted" : "LogMinerStarted"));
        }
        catch (Exception ex)
        {
            SetStatus("StatusStartFailed");
    ...
    internal static bool TryParsePoolEndpoint(string? value, out string host, out int port)
        => MinerService.TryParsePoolEndpoint(value, out host, out port);
    ...
    private async Task StopAsync()
    {
        try
        {
            ...
            await _miner.StopAsync(_activeMinerConfig.StopTimeoutSeconds, _lifetime.Token);
    ...
    private async Task MonitorAsync(CancellationToken ct)
    {
        ...
        try
        {
            while (await timer.WaitForNextTickAsync(ct))
            {
                ...
                if (_miner.IsRunning) api = await _statsApi.TryGetAsync(_activeMinerConfig.StatsAddress, _activeMinerConfig.StatsPort, ct);
    ...
    private void ApplyMonitoring(IReadOnlyDictionary<int, GpuMetrics>? metrics, MinerApiStats? api, bool? nodeReachable)
    {
        ...
        if (api is not null)
        {
            var apiHashrateMh = api.TotalHashrateHs > 0 ? api.TotalHashrateHs / 1_000_000d : _fallbackStats.HashrateMh;
            Hashrate = $"{apiHashrateMh:0.00} MH/s";
            ...
            var elapsedSeconds = _minerStartedAtUtc == DateTime.MinValue ? 0 : Math.Max(0, (long)(DateTime.UtcNow - _minerStartedAtUtc).TotalSeconds);
            Uptime = FormatUptime(api.UptimeSeconds > 0 ? api.UptimeSeconds : elapsedSeconds);
            if (IsSoloMode)
            {
                NodeSyncKnown = true;
            ...
            var suprnovaPreparing = _activeMinerBackend == MinerBackend.Suprnova
                && apiHashrateMh <= 0.0001
                && DateTime.UtcNow - _minerStartedAtUtc < TimeSpan.FromMinutes(30);
            if (_miner.IsRunning && (IsPoolMode || api.Synced) && !suprnovaPreparing)
            {
                _activeBlockers.Clear();
                SetStatus("StatusMining");
            }
            else if (_miner.IsRunning && suprnovaPreparing && _activeBlockers.Count == 0)
            {
                SetStatus("StatusMiningStarting");
                if (string.IsNullOrWhiteSpace(ServiceStatusText)) ServiceStatusText = T("SuprnovaPreparingModels");
            }
        ...
        else if (_miner.IsRunning)
        {
            ...
            if (_minerStartedAtUtc != DateTime.MinValue)
                Uptime = FormatUptime(Math.Max(0, (long)(DateTime.UtcNow - _minerStartedAtUtc).TotalSeconds));
        }
        ...
        var hashMh = api is { TotalHashrateHs: > 0 } ? api.TotalHashrateHs / 1_000_000d : _fallbackStats.HashrateMh;
        Efficiency = totalPower > 0 ? $"{hashMh / totalPower:0.000} MH/W" : "— MH/W";
        var suprnovaStartupGrace = _activeMinerBackend == MinerBackend.Suprnova
            && _miner.IsRunning
            && hashMh <= 0.0001
            && DateTime.UtcNow - _minerStartedAtUtc < TimeSpan.FromMinutes(30);
        var miningExpected = _activeMinerBackend == MinerBackend.Suprnova
            ? _miner.IsRunning && !suprnovaStartupGrace
            : _statusKey == "StatusMining";
        ApplyMiningHealth(_miningHealth.Evaluate(DateTime.UtcNow, _miner.IsRunning,
            miningExpected, hashMh, maximumTemperature, api?.OpoiChallengeActive == true));
        QueueEsp32Telemetry(api, selected.Length, hashMh, maximumTemperature, totalPower);
        QueueTurzxFrame();
    }

    private void QueueEsp32Telemetry(MinerApiStats? api, int gpuCount, double hashrateMh,
        double? maximumTemperature, double totalPower)
    {
        var state = TrayState switch
        {
            TrayIconState.Error => Esp32DisplayState.Error,
            TrayIconState.Warning => Esp32DisplayState.Warning,
            TrayIconState.Mining => Esp32DisplayState.Mining,
            _ when _statusKey == "StatusStopped" => Esp32DisplayState.Stopped,
            _ => Esp32DisplayState.Waiting
        };

        var elapsedSeconds = _minerStartedAtUtc == DateTime.MinValue ? 0 : Math.Max(0, (long)(DateTime.UtcNow - _minerStartedAtUtc).TotalSeconds);
        _esp32Telemetry.Publish(new Esp32DisplaySnapshot(
            state,
            gpuCount,
            _miner.IsRunning ? Math.Max(0, hashrateMh) * 1_000_000d : 0,
            maximumTemperature ?? 0,
            totalPower,
            Accepted,
            Rejected,
            api is { UptimeSeconds: > 0 } ? api.UptimeSeconds : elapsedSeconds,
            _activeMinerBackend == MinerBackend.Suprnova ? "Keryx Suprnova" : "Keryx Miner"));
    }
    ...
    private void AddLogCore(string line, bool isMiner)
    {
        ...
        if (isMiner)
        {
            ...
            if (_miner.IsRunning && _fallbackStats.HashrateMh > 0)
            {
                if (Hashrate == "0.00 MH/s") Hashrate = $"{_fallbackStats.HashrateMh:0.00} MH/s";
                if (_statusKey == "StatusMiningStarting") SetStatus("StatusMining");
                if (_activeMinerBackend == MinerBackend.Suprnova && ServiceStatusText == T("SuprnovaPreparingModels")) ServiceStatusText = "";
            }
    ...
    public void SetLanguage(string? language)
    {
        ...
        foreach (var miner in MinerOptions) miner.SetLanguage(normalized);
        foreach (var model in TurzxModelOptions) model.SetLanguage(normalized);
        foreach (var gpu in Gpus) gpu.SetLanguage(normalized);
        Status = T(_statusKey); RefreshNodeSyncText(); Raise(nameof(LogPauseText)); Raise(nameof(StartButtonText)); Raise(nameof(SuprnovaMinerStatusText)); Raise(nameof(SuprnovaMinerStatusForeground));
    ...
    private async Task OnNodeExitedAsync(int code)
    {
        ...
        if (!_nodeStopRequested && !_shutdownStarted)
        {
            ...
            {
                _minerStopRequested = true;
                try { await _miner.StopAsync(_activeMinerConfig.StopTimeoutSeconds, CancellationToken.None); } catch { }
                await ShutdownManagedKuboAsync();
                await RestorePowerAfterUnexpectedExitAsync();
    ...
    {
        Raise(nameof(IsMinerRunning)); Raise(nameof(IsNodeRunning)); Raise(nameof(HasActiveProcesses)); Raise(nameof(ConfigurationEnabled)); Raise(nameof(ConnectionConfigurationEnabled)); Raise(nameof(MiningModeConfigurationEnabled)); Raise(nameof(NodeControlsEnabled)); Raise(nameof(TrayState)); RefreshCommands();
    }
    ...
    {
        _settings.Language = Language; _settings.Wallet = Wallet; _settings.MinerBackend = IsSuprnovaMiner ? "suprnova" : "reference"; _settings.MiningMode = IsPoolMode ? "pool" : "solo";
        _settings.NodeAddress = NodeAddress; _settings.NodePort = NodePort; _settings.PoolAddress = PoolAddress;
        _settings.SuprnovaMinerPath = SuprnovaMinerPath; _settings.SuprnovaWorker = SuprnovaWorker; _settings.SuprnovaPoolAddress = SuprnovaPoolAddress;
    ...
    public async Task ShutdownAsync()
    {
        ...
        {
            try { await _monitorTask.WaitAsync(TimeSpan.FromSeconds(6)); }
        ...
        try { await _turzx.DisposeAsync().AsTask().WaitAsync(TimeSpan.FromSeconds(6)); } catch { }
        try { await _esp32Telemetry.DisposeAsync().AsTask().WaitAsync(TimeSpan.FromSeconds(6)); } catch { }
