...
var argsList = MinerService.BuildArguments(minerConfig, MinerBackend.Reference, "keryx:testaddress1234567890", MinerConnection.Solo("127.0.0.1", 22110), selections);
Check(argsList.Contains("--force-model") && argsList.Contains("light,very-high"), "non-contiguous multi-GPU model order");
var suprnovaArgs = MinerService.BuildArguments(minerConfig, MinerBackend.Suprnova,
    "keryx:testaddress1234567890", MinerConnection.Pool("stratum+tcp://krx.suprnova.cc:4404"), selections, "rig_01");
Check(suprnovaArgs.SequenceEqual(new[]
{
    "-a", "keryx:testaddress1234567890.rig_01",
    "-s", "stratum+tcp://krx.suprnova.cc:4404",
    "--api-bind", "127.0.0.1:3338",
    "--no-tui", "--force-model", "light,very-high"
}), "Suprnova command-line contract");
Check(MinerService.TryParsePoolEndpoint("stratum+ssl://krx.suprnova.cc:4405", out var sslHost, out var sslPort)
      && sslHost == "krx.suprnova.cc" && sslPort == 4405, "Suprnova SSL endpoint validation");
Check(MinerService.IsValidSuprnovaWorker("rig_01") && !MinerService.IsValidSuprnovaWorker("rig.01"), "Suprnova worker validation");
...
var telemetry = Esp32TelemetryProtocol.Format(42, new Esp32DisplaySnapshot(
    Esp32DisplayState.Mining, 4, 5_820_000, 61, 179.8, 33, 0, 3661, "Keryx Miner"));
Check(telemetry == "KX1|42|mining|4|5820000|61|179.8|33|0|3661|Keryx Miner",
    "ESP32 telemetry packet");
var sanitizedTelemetry = Esp32TelemetryProtocol.Format(43, new Esp32DisplaySnapshot(
    Esp32DisplayState.Warning, 99, double.NaN, 999, -10, -1, -2, -3, "Bad|Miner\n"));
Check(sanitizedTelemetry == "KX1|43|warning|32|0|150|0|0|0|0|BadMiner",
    "ESP32 telemetry validation and privacy delimiter");
...
if (failures.Count == 0)
{
    Console.WriteLine("All Keryx Control v0.8.2 TURZX + ESP32 + Suprnova smoke tests passed.");
    return 0;
}
